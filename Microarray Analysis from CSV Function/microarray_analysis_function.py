...
#Define merge function for dictionaries
...
def micro_analysis(accession_id, control_samples, treated_samples):
    ...
    dict1 = {}
    with open(WORKDIR + '/p2g8.txt') as f:
        for line in f:
            line = line.strip()
    ...
    #Median of values with repeated gene symbols
    data = data.groupby(by=data.index, axis=0).apply(lambda g: g.median() if isinstance(g.iloc[0,0], numbers.Number) else g.iloc[0])
